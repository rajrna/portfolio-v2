...
export default function FlickerCanvas() {
  ...
  useEffect(() => {
    ...
    let nextIdleGlitch = Date.now() + getRandomIdleDelay();
    ...
    function drawFlicker() {
      ...
      for (let y = 0; y < canvas.height; y += 3) {
        let offsetX = Math.sin((y + time) * 0.02) * 2;
        ...
        ctx.fillRect(offsetX + 1, y, canvas.width, 1);
    ...
    function animate() {
      ...
      if (!glitchActive && now > nextIdleGlitch) {
        triggerGlitch();
        nextIdleGlitch = now + getRandomIdleDelay();
      }
    ...
    const handleMouseEnter = () => triggerGlitch();
